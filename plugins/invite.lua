...
--vardump(result)
  local user = 'user#id'..result.peer_id
	...
	if is_banned(result.id, extra.chatid) then 
        send_large_msg(chat, 'کاربر مسدود است.')
        send_large_msg(channel, 'کاربر مسدود است.')
	elseif is_gbanned(result.id) then
	    send_large_msg(chat, 'کاربر سراسری مسدود است.')
		send_large_msg(channel, 'کاربر سراسری مسدود است.')
	else    
